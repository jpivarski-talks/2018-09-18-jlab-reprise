...
\vspace{0.25 cm}
\begin{itemize}\setlength{\itemsep}{0.25 cm}
\item PAW/HBOOK histograms were indexed by {\it integers} (!)
\item ROOT histograms are indexed by {\it string names}
\item ROOT TTrees are indexed by {\it integer entries}
\item Excel spreadsheets are indexed by {\it integer row/column IDs}
\item SQL tables are indexed by {\it unordered sets}
\item Pandas DataFrames are indexed by {\it ordered, structured series}
\end{itemize}
\end{frame}
